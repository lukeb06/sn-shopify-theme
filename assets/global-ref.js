...
export default function (context) {
  ...
  const stickyHeader = () => {
    $("[data-sticky-header]")
      ...
      .each((i, el) => {
        ...
        const $placeholder = $('<div class="stickyHeader-placeholder"></div>')
          .show()
  ...
  try {
    let ra = document
      .querySelector('[data-content-region="product_below_addtocart--global"]')
  ...
  try {
    ...
    if (!document.getElementById("rewardsButton")) throw "No rewards button";
    if (!document.getElementById("plantingGuideButton"))
  ...
  function isPlantOrTreePage() {
    ...
    if (breadcrumbsElement) {
      ...
      return (
        breadcrumbsText.includes("Plants") || breadcrumbsText.includes("Trees")
      );
    }
    return false;
  }
  ...
  async function submitNotifyStockForm(e) {
    ...
    const response = await fetch(
      `https://termite-enormous-hornet.ngrok-free.app/stock_notify`,
      {
        method: "POST",
  ...
  try {
    ...
    if (!document.querySelector("#tab-addition")) throw "Not a product page";
      ...
      return new Promise((resolve, reject) => {
        fetch(endpoint)
          ...
          .then((r) => {
            ...
            let addToCartForm = dom.querySelector(".form--addToCart");
            if (!addToCartForm)
              return resolve({
                product_id: null,
            ...
            if (formField) {
              attribute_id = formField.querySelector(".form-radio").name;
              ...
              options.forEach((pair) => {
                product_options[pair.option.textContent.trim()] =
                  pair.radio.value;
              });
            }
    ...
    (async () => {
      ...
      IDS.raisedBedMix = getProductDetailsFromEndpoint(ENDPOINTS.raisedBedMix);
    ...
    function getProductDetailsFromIDsUsingAttributeID(attribute_id) {
      let targetAttributeId = `attribute[${Object.keys(attribute_id)[0]}]`;
    ...
    const startTheShow = () => {
      ...
      const convertJSONToFormDataString = (_json) => {
        ...
        formDataString += "------WebKitFormBoundary8Ub3q4FeZPdXd7MI--\r\n";
      ...
      const extractProductDataWithOptions = (gData) => {
        ...
        for (const [key, value] of Object.entries(gData.options)) {
          newOptions[encodeURIComponent(key)] = encodeURIComponent(value);
        }
      ...
      const extractProductCardData = (gData) => {
        return new Promise(async (resolve, reject) => {
          ...
          if (gData.product_id == IDS.raisedBedMix.product_id) {
            ...
            thumbnail = response.image.data.replaceAll("{:size}", "500x500");
      ...
      const biotoneDecimal = [0.33, 0.33, 0.5, 0.66, 1, 1, 1.33, 2, 2, 2, 8];
      const raisedBedDecimal = [0.066, 0.2, 0.25, 0.33, 0.5, 1, 2, 3, 3, 4, 5];

      const __form = document.querySelector(".form--addToCart");
      const __quantity = __form.querySelector("input[name='qty[]']").value;
      ...
      async function createRecommendedPopup(_g, gs) {
        ...
        cartDataDOM.querySelector(".previewCart").style.display = "none";

        const addAllToCartButton = cartDataDOM.querySelector(".checkout");
        // addAllToCartButton.textContent = "Add All to Cart";

        cartDataDOM.querySelector(".previewCartAction-totals").style.margin =
          "0";

        const OG_CART_ITEM = cartDataDOM.querySelector(".previewCartItem");
        OG_CART_ITEM.remove();
        ...
        const createCartItem = (
          ...
          cartItem.querySelector(".previewCartItem-name a").textContent = name;
          cartItem
          ...
          cartItem.querySelector(".previewCartItem-name a").onclick = (e) =>
            e.preventDefault();
          cartItem
            .querySelector(".previewCartItem-name a")
            .removeAttribute("href");
          cartItem.querySelector(".previewCartItem-name a").style.cursor =
            "default";
          cartItem.querySelector(
            ".previewCartItem-name a"
          ).style.textDecoration = "none";
          cartItem.querySelector(".previewCartItem-name a").style.color =
            "black";
          cartItem.querySelector(".previewCartItem-price span").textContent =
            instock
              ? `${quantity < 2 ? "" : quantity + " x "}$${price}`
              : "Out of Stock";
          cartItem.querySelector(".previewCartItem-image img").srcset = "";
          cartItem.querySelector(".previewCartItem-image img").dataset.srcset =
            "";
          cartItem
        ...
        const createProduct = (productData) => {
          ...
          const contentDiv = newItem.querySelector(".previewCartItem-content");
          ...
          previewCart.querySelector(".previewCartList").appendChild(newItem);
        ...
        [biotoneData, raisedBedMixData, permaTillData] = await Promise.all([
          biotoneData,
          raisedBedMixData,
          permaTillData,
        ]);
        ...
        const _quantity = _form.querySelector("input[name='qty[]']").value;

        const checkedOptionEl = document.querySelector(".form-radio:checked");
        ...
        Array.from(previewCart.querySelectorAll(".addToCartButtonPop")).forEach(
          (button) => {
            button.addEventListener("click", (e) => {
              ...
              originalAttributes.borderRadius = button.style.borderRadius;
              originalAttributes.border = button.style.border;
              originalAttributes.borderTop = button.style.borderTop;
              originalAttributes.backgroundColor = button.style.backgroundColor;
              ...
              let productData = JSON.parse(
                button.parentElement.parentElement.dataset.productData
              );
              addToCart(
                ...
                button.style.borderRadius = originalAttributes.borderRadius;
                button.style.border = originalAttributes.border;
                button.style.borderTop = originalAttributes.borderTop;
                button.style.backgroundColor =
                  originalAttributes.backgroundColor;
                button.style.padding = originalAttributes.padding;
                button.style.animation = originalAttributes.animation;
                button.style.margin = originalAttributes.margin;
                button.style.marginTop = originalAttributes.marginTop;
                button.disabled = true;

                new Promise((resolve) => setTimeout(resolve, 3000)).then(() => {
                  button.textContent = originalAttributes.textContent;
                  button.disabled = false;
                });
              });
            });
        ...
        addAllToCartButton.addEventListener("click", () => {
          ...
          addAllToCartButton.style.animation = "spin 1s linear infinite";
    ...
    function createTheButton() {
      ...
      BUTTON_CREATED = true;
      const optionsWrapper = document.querySelector(".productView-options");
      function buildButton() {
        let btn = document.createElement("button");
        btn.textContent = "Recommended Products";
        btn.className = "recommended-products-option button button--primary";
    ...
    if (
      document.querySelector(".breadcrumbs").textContent.includes("Trees") ||
      document.querySelector(".breadcrumbs").textContent.includes("Plants")
    ) {
      let btn = createTheButton();
      btn.disabled = true;

      Array.from(document.querySelectorAll(".form-radio")).forEach((radio) => {
        radio.addEventListener("click", () => (btn.disabled = false));
      });
    }
  } catch (e) {
    console.warn(e);
  ...
  try {
    ...
    getPromotionProducts().then((promotionProducts) => {
      ...
      if (prod) {
        ...
        promo.textContent = promotionProducts["promotions"][prod.promotion];
        info.after(promo);
  ...
  try {
    const productImgContainers = Array.from(document
  ...
  const handleHeaderGrid = (r) => {
    ...
    const section = document.querySelector(".beautify__section--header-grid");
    const regionA = section.querySelector(".region-a");
    const regionB = section.querySelector(".region-b");

    const headerA = regionA.querySelector(".header").querySelector("strong");
  ...
  try {
    ...
    let grids = [
      newGrid(".beautify__section--3x1grid", "3x1_config", (data, child) => {
      ...
      newGrid(".beautify__section--blocks", "blocks_config", (data, child) => {
        const image_url = data["image_url"];
        const href = data["href"];
        const alt = data["alt"];
      ...
      newGrid(".beautify__section--4x2grid", "4x2_config", (data, child) => {
        const image_url = data["image_url"];
        const href = data["href"];
  ...
  try {
    ...
    function initRadios() {
      document.querySelectorAll(".gift-radio").forEach((radio) => {
        radio.onclick = () => {
          document.querySelector(".gift-radio:checked").checked = false;
          radio.checked = true;
